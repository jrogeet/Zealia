...
<body class="static bg-white1 font-synereg min-w-[78rem]">
    ...
    <div class="relative w-full h-full text-center top-[5.8rem] pt-8 mb-32                    ">
        ...
        <div class="relative flex left-1/2 transform -translate-x-1/2 w-full h-5/6 min-h-[47rem] px-6">
            <!-- left box -->
            <div class="block border border-black rounded-2xl w-5/12 h-[70rem] m-auto mt-10 bg-white2 text-left p-[4%] pt-2 pl-[4%]">
                <h5 class="text-xl text-grey2 mt-16">Name</h5>
                ...
                <h1 class="text-3xl ml-4 mb-8"><?php echo "{$_SESSION['user']['school_id']}"; ?></h1>

                <h5 class="text-xl text-grey2 mt-24">Change Password</h5>
                
                <form method="POST" action="/account">
                    <input class="border border-grey2 p-2 h-10 rounded-lg mt-2 ml-4" type="text" placeholder="Current Password" name="cur_pass" required></input>
                    <div class="flex mt-2 ml-4">
                        <input class="border border-grey2 p-2 h-10 rounded-lg mt-2" type="text" placeholder="New Password" name="new_pass" required></input>
                        <input class="border border-grey2 p-2 h-10 rounded-lg mt-2 ml-4" type="text" placeholder="Confirm New Password" name="conew_pass" required></input>
                    </div>
                    <button class="relative left-1/2 transform -translate-x-1/2 border border-black1 rounded-lg px-8 h-10 mt-32 bg-orange1 text-black1">Save Changes</button>
                </form>
            ...
            <div class="block border border-black rounded-2xl w-5/12 h-[70rem] m-auto mt-10 bg-white2 text-left p-[4%] pt-2 pl-[4%]">
                ...
                <?php else:?>
                    <h1 class="relative top-72 transform -translate-y-1/2 font-synemed text-4xl text-center">You haven't taken the test!</h1>
                    <a href="/test"><button class="relative top-72 border border-grey2 rounded-2xl w-40 h-12 bg-orange1 font-synemed text-xl left-1/2 transform -translate-x-1/2">Take Test</button></a>
                <?php endif;?>
