...
<body class="bg-white1 block">
    ...
    <script>
        ...
        function changeGroup(name, start, end, zone, card) {
            let memInd = 0;
            let trash
            let user
            ...
            if (start == end) {
                ...
                for (let member of groups[end]) {
                    if (member.includes(user[2])) {
                        ...
                        let el2 = document.getElementById(oldRoleID);
                        el2.id = newRoleID;
            ...
            for (let member of group){// + = drag icon
                groupHTML = `<div class="flex w-full h-[6rem] bg-white1 border-t border-black1 cursor-grab active:cursor-grabbing text-left draggable" id="${member[1]} ${groupNum}" draggable="true">
                                <div class="w-2/3 p-2 pl-6">
                                    ...
                                    <h1 class="font-synemed text-xl text-grey2" id="role ${member[1]} ${groupNum}">${member[2]}</h1>
                                </div>
        ...
        dropZones.forEach(function(zone) {
            ...
            zone.addEventListener('drop', function(event) {
                ...
                endGroup = index
                changeGroup(name, inGroup, endGroup, zone, card)
                console.log(groups);
